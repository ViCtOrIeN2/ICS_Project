...
def updating_writer(a):
    ...
    current_command = context[slave_id].getValues(4, 1, 1)[0] / 65535.0*100.0

    s.send('{"request":"write","data":{"inputs":{"temperature":'+repr(current_command)+'}}}\n')

    # import pdb; pdb.set_trace()
    #s.send('{"request":"read"}')
    data = json.loads(s.recv(1500))
    temperature = data["outputs"]["temperature"]
    thermostat_mode = int(data["states"]["thermostat_mode"])
    # import pdb; pdb.set_trace()
    context[slave_id].setValues(4, 1, [temperature,thermostat_mode])
